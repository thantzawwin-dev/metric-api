...
const KongServiceMetrics = ({ metrics = [] }) => {
  const KongServiceRouteNames = takeByNameOutOfKongMetricPropertyObj(metrics, 'route')
                      ...
                      {kongMetricUIConfig[metric['title']].map((uiConfig, idx) => 
                        <Suspense key={idx} loading={<div>loading</div>}>
                          <uiConfig.element 
                            ...
                            values={
                              metric['values'].filter(value => 
                                value.metricName === uiConfig.metricName && value.metricProperty.route === sRoutes
                              )
                            } 
                          />
                        </Suspense>
